...
//    let grole = message.guild.roles.find(`name`.toLocaleLowerCase(), role);
//    if (!grole) return message.reply("Couldn't find role.");
    ...
    if(message.member.roles.has(id)) return message.reply("You already have that role.");
    await(message.member.addRole(id));
    message.channel.send(`You now have ${role}`);
